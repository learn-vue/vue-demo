<template>
	<div class="dev-data-index">
			...
			<div class="search-bar pull-right">
				...
				<div class="form-group pull-right">
					<select class="form-control" v-model="adslotId" @change="load">
						<option v-for="adslot in adslots" value="{{adslot.id}}">{{adslot.text}}</option>
					</select>
				</div>
				<!-- 应用下拉 -->
				<div class="form-group pull-right">
		...
		<div class="chart-bar">
			...
			<div v-show="adslotId!='-1'" id="chart" class="chart-cnt"></div>
		
			<div v-show="adslotId=='-1'" class="chart-empty">抱歉，暂无数据！</div>
		...
		<table class="table table-bordered table-striped table-hover">
			<thead>
			  <tr>
				...
				<th class="text-center">请求数</th>
				<th class="text-center">展现数</th>
				<th class="text-center">点击数</th>
				<th class="text-center">点击收入</th>
				<th class="text-center">点击率</th>
				<th class="text-center">千次展现收入</th>
			  </tr>
			</thead>
			<tbody>
		...
		<div class="pager-box">
			...
			<a v-if="list.length > 0" href={{exportURL}} target="_blank" class="btn btn-primary btn-common btn-export">导出Excel</a>
	...
	module.exports = {
		...
		computed:{
			...
			exportURL:function(){
				...
				var url=ApiUtil.url('/v1/dev/stats/adSlotOfDay/'+this.appId+'/'+this.adslotId+'/export');
		...
		methods:{
			...
			loadApps:function(){
				...
				this.$http.get('/v1/dev/names/apps').then(function(res){
			...
			loadAdslots:function(){
				...
				var resource = this.$resource('/v1/dev/names/apps/{appId}/adslots');
			...
			pageTo:function(pageNum){
				...
				var resource = this.$resource('/v1/dev/stats/adSlotOfDay/{appId}/{adslotId}/{pageNum}/{pageSize}');
			...
			load:function(){
	 			...
	 			if(this.appId=="-1" || this.adslotId=="-1"){
				...
				this.$http.get('/v1/dev/stats/adSlotOfDay/{appId}/{adslotId}',params).then(function(res){
...
<style lang="less">
	...
	.dev-data-index .crumbs-title{
		margin-bottom: 0;
	    margin-top: 9px;
	}
	
	.dev-data-index .search-empty{
		text-align:center;
		padding:200px 0;
	...
	.dev-data-index .search-bar{
		width:80%;
	}
	
	.dev-data-index .search-bar .form-group{
		margin: 0 0 0 15px;
    	width: 30%;
	}

	.dev-data-index .chart-bar{
		height:400px;
		border: 1px solid #ddd;
		margin-top: 20px;
	}
	
	.dev-data-index .chart-empty{
		padding-top:150px;
		text-align:center;
	}
	
	.dev-data-index .chart-cnt{
		height:100%;
	}
	
	.dev-data-index .table{
		margin-top:30px;
	}

	.dev-data-index .pager-box{
		position:relative;
	}
	
	.dev-data-index .pagination-summary{
		margin-right:30px;
		float:right !important;
	}
	
	.dev-data-index .btn-export{
		position:absolute;
		top: 0;
	}
</style>
