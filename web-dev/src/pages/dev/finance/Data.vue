<template>
	<div class="dev-finance-data">
		
		<div class="crumbs">
			<h4 class="crumbs-title">开发者财务概览</h4>
		</div>
		...
		<div class="search-bar clearfix">
			...
			<h4 class="pull-left search-title">收入结算记录</h4>
	...
	module.exports = {
		...
		computed:{
			...
			exportURL:function(){
				...
				var url=ApiUtil.url('/v1/dev/finances/settles/export');
		...
		ready:function(){
			...
			this.$http.get("/v1/dev/stats/finances/balance").then(function(res){
				...
				if(data.ret == 1){
					...
					this.earnings = data.result.earnings;
				}
		...
		methods:{
			...
			pageTo:function(pageNum){
				...
				var resource = this.$resource('/v1/dev/finances/settles/{pageNum}/{pageSize}');
...
<style>
	...
	.dev-finance-data .crumbs-title{
		margin-bottom: 20px;
		margin-top: 0;
	}

	.dev-finance-data .date-range{
		width: 226px;
		margin-left:20px;
		margin-top:2px;
	}
	
	.dev-finance-data .col-1-2{
		width:50%;
		padding:0 8px;
	}
	
	.dev-finance-data .sub-block-first{
		padding-left:0;
	}
	
	.dev-finance-data .sub-block-last{
		padding-right:0;
	}
	
	.dev-finance-data .sub-block{
		border:1px solid #EAEDF1;
		text-align:center;
		background-color:#f5f6fa;
	}
	
	.dev-finance-data .stat-bar{
		margin: 30px 0;
	}
		
	.dev-finance-data .stat-bar h4{
		margin: 0;
		padding: 20px 0;
		display: inline-block;
	}
		
	.dev-finance-data .sub-block p{
		font-size: 18px;
		padding-top:10px;
	}
	
	.dev-finance-data .spliter{
		margin:0 5%;
		border-top:1px solid #eaedf1;
	}
	
	.dev-finance-data .search-bar{
		margin-bottom:20px;
	}
	
	.dev-finance-data .search-title{
		font-size:14px;
	}
	
	.dev-finance-data .pager-box{
		position:relative;
	}
	
	.dev-finance-data .search-empty{
		text-align:center;
		padding:100px 0;
	...
	.dev-finance-data .pagination-summary{
		margin-right:30px;
		float:right !important;
	}
	
	.dev-finance-data .btn-export{
		position:absolute;
		top: 0;
	}
</style>