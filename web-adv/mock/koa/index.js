...
app.use(function *(next) {
    ...
    if (method === 'OPTIONS') 
    	...
    	for(var i=0,l=rules.length;i<l;i++){
    		...
    		rule.urlKeyCount=Math.min(keys.length,values.length);
    	...
    	if(right.length){
    		...
    		for(var i=0,l=right.length;i<l;i++)
    			if(right[i].prority>maxPrority)
    				maxPrority=right[i].prority;
    		
    		for(var i=0,l=right.length;i<l;i++)
    			if(right[i].prority==maxPrority)
    				rightRules.push(right[i]);
    		
    		//URL路径参数最精确匹配
    		if(rightRules.length > 1)
    			rightRules=rightRules.sort(function(a,b){
    				
    				return a.urlKeyCount - b.urlKeyCount ;
    			
    			});
    		
    		// 根据优先级计算
    		var rule=rightRules[0];
    		for(var i=1,l=right.length;i<l;i++)
    			if(right[i].prority > rule.prority)
    				rule=right[i];
 
	    	console.log('api file:',fileMap[method+":"+rule.url]);
	    	
	    	var data=require(fileMap[method+":"+rule.url]).res;
