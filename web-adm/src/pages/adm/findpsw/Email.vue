				...
				<validator name="validation" >
					...
					<form novalidate name="signForm">
						...
						<div v-if="sendState==false" class="clearfix button-group">
							<a type="button" @click="sendAuthCode" :class="{'send-btn':true,btn:true,'btn-common':true,'btn-primary':true,disabled:$validation.email.email||$validation.email.required}">发送验证码</a>
						</div> 
	...
	module.exports = {
		...
		methods:{
			...
			sendAuthCode:function(){
				...
				this.$http.post('/v1/adm/findpsw/sendEmail', params).then(function(response){
					this.unmask();
			...
			nextStep:function(){
				...
				this.$http.post('/v1/adm/findpsw/confirmEmail', params).then(function(response){
					this.unmask();
					var data = response.data;
					if (data.ret != 1) {
						this.sendState = false;
					...
					this.$route.router.go({
						path:'/adm/findpsw/reset?userId=' + userId
					});
...
<style>
	...
	.adm-findpsw-email .fz12{
		font-size: 12px;
	}

	.adm-findpsw-email .margintop15{
		margin-top: 15px;
	}

	.adm-findpsw-email .findpsw-logo{
		text-align: center;
		color: #09c;
		margin: 50px 0;
	}

	.adm-findpsw-email .findpsw-step-box{
		text-align: center;
		height: 35px;
	}

	.adm-findpsw-email .findpsw-step{
		display: inline-block;
	}

	.adm-findpsw-email .findpsw-step.active{
		color: #09c;
	}

	.adm-findpsw-email .step-line{
		margin: 0 20px;
	}

	.adm-findpsw-email .findpsw-content{
		border: 1px solid #c2c3c7;
		margin-bottom: 50px;
	}
	
	.adm-findpsw-email .findpsw-tab{
		width: 100%;
		text-align: center;
		display: inline-block; 
		height: 42px;
	...
	.adm-findpsw-email .findpsw-tab:hover{
		text-decoration: none;
	}

	.adm-findpsw-email .findpsw-tab.active{
		background: #09c;
		color: #fff;
	}

	.adm-findpsw-email .findpsw-steping{
		color: #09c;
		background: #fff;
		padding: 10px 20px;
	}
	
	.adm-findpsw-email .findpsw-body{
		background: #fff;
		padding: 65px 85px;
	}

	.adm-findpsw-email .next-btn{
		margin: 20px 0;
	    width: 150px;
	    margin-left: 25px;
	    padding: 10px 12px;
	...
	.adm-findpsw-email .errors{
		color: red;
		margin-top: 5px;
		margin-left: 105px;
	}

	.adm-findpsw-email .font16{
		font-size: 16px;
	}
	
	.adm-findpsw-email .form-control{
		width: 330px;
		display: inline-block;
	}

	.adm-findpsw-email .form-group label{
		width: 100px;
		text-align: right;
		padding-right: 15px;
		font-weight: normal;
	...
	.adm-findpsw-email .button-group{
		margin-left: 105px;
    	width: 330px;
	}
	/*进度条部分*/
	.adm-findpsw-email .findpsw-title{
		text-align: center;
		padding-left: 0;
		height: 42px;
		margin-bottom: 0;
	...
	.adm-findpsw-email .step-one{
		width: 33.33%;
		background: #09c;
		color: #fff;
	}
	
	.adm-findpsw-email .step-two{
		width: 33.33%;
		background: #cccfd3;
		color: #fff;
	}

	.adm-findpsw-email .step-three{
		width: 33.33%;
		background: #cccfd3;
		color: #fff;
	}

	.adm-findpsw-email .step-one .triangle-right{
		border-color: transparent transparent transparent #09c;
	}

	.adm-findpsw-email .step-three .triangle-right,.adm-findpsw-email .step-two .triangle-right{
